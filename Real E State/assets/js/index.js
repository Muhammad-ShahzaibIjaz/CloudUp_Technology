...
const searchMinPrice = document.querySelector('#searchMinPrice');
const searchMaxPrice = document.querySelector('#searchMaxPrice');
...
searchMinPrice.addEventListener('keyup', ()=>{
    let arr = [];
    let searchedVal = searchMinPrice.value;
    arr = priceTypes.filter(data => {
        return data.toLowerCase().startsWith(searchedVal);
    }).map(data => `<li>${data}</li>`).join("");
    priceOption.innerHTML = arr ? arr : `<p style='padding:0px 10px;color:#000!important' >Result Not Found</p>`;
});

searchMaxPrice.addEventListener('keyup', ()=>{
    let arr = [];
    let searchedVal = searchMaxPrice.value;
    arr = priceTypes.filter(data => {
        return data.toLowerCase().startsWith(searchedVal);
    }).map(data => `<li>${data}</li>`).join("");
    priceOption1.innerHTML = arr ? arr : `<p style='padding:0px 10px;color:#000!important' >Result Not Found</p>`;
});
...
let priceTypes = ["30,000","40,000","50,000","60,000","70,000","80,000","90,000","100,000","110,000","120,000","130,000","140,000","150,000","160,000","170,000","180,000","190,000","200,000","225,000","250,000","300,000","350,000","400,000","500,000","750,000","1,000,000"];
...
if(window.innerWidth > 991){
    window.onscroll = function() {
        const dropdownMenu = document.querySelector('.dropdown-hidden-menu');
